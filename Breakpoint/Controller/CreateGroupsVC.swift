    ...
    @IBAction func doneBtnPressed(_ sender: Any) {
        if titleTxtField.text != "" && descriptionTxtField.text != "" {
            DataService.instance.getIds(forUsernames: chosenUserArray, handler: { (idsArray) in
                ...
                DataService.instance.createGroup(withTitle: self.titleTxtField.text!, andDescription: self.descriptionTxtField.text!, forMembers: userIds, handler: { groupCreated in
                    if groupCreated {
                        self.dismiss(animated: true, completion: nil)
                    } else {
                        print("Could not be created group. please try again")
                    }
                })
            })
        }
